...
export function EventListTools() {
  ...
  const view = params.get('eventView') || 'current';
  ...
  return <div className="masca-event-tools masca-section-shell">
    ...
    {view === 'archived' && <p>Archived events are kept for your records and reports. Restore an event to return it to the other lists, or select archived events and choose Delete to permanently remove them, their history and their entries in reports.</p>}
    {reports && <section className="masca-event-report" aria-label="Event reports">
      ...
      {report && <div className="masca-event-report__output">
        ...
        {!report.total ? <p>No events in this period.</p> : <div className="masca-event-report__table"><table><thead><tr><th>Event</th><th>Organisation</th><th>State</th><th>Lifecycle</th></tr></thead><tbody>{report.rows.map(row => <tr key={row.id}><td>{row.title}</td><td>{row.organisation}</td><td>{row.state}</td><td>{row.lifecycle}</td></tr>)}</tbody></table></div>}
      </div>}
    </section>}
  </div>;
}
