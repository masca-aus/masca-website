  ...
  return {
    ...
    hooks: { afterRead: [async ({ data, req, context }: Parameters<FieldHook>[0]) => {
      ...
      const [live, latest, stages] = await Promise.all([
        req.payload.findByID({ collection, id: data.id, draft: false, select: collection === 'events' ? { _status: true, reviewStatus: true } : { _status: true, closes: true, added: true }, depth: 0, req: { ...req, context: readContext }, context: readContext, overrideAccess: false }),
        req.payload.findByID({ collection, id: data.id, draft: true, select: { _status: true, closes: true, added: true }, depth: 0, req: { ...req, context: readContext }, context: readContext, overrideAccess: false }),
        collection === 'events'
