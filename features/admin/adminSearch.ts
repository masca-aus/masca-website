...
import { statusLabels } from './cmsStatus.ts';
...
type Plan={collection:SearchCollection;terms:string[];title:string;search:string;rank:boolean;statusSort?:boolean};
...
export function withAdminSearch(collection:SearchCollection, base?:BaseFilter):BaseFilter {
 return async args => {
  ...
  const terms=searchTerms(search);
  const sort=args.req.query?.sort || args.sort;
  const statusSort=(collection==='events'||collection==='careers') && (sort==='cmsStatus'||sort==='-cmsStatus');
  if(!args.req.user || (!terms.length && !statusSort)) return constraint;
  ...
  pending.set(args.req,{collection,terms,search,rank,statusSort:statusSort ? sort==='-cmsStatus' : undefined,title:collection==='events'||collection==='careers'?'title':'name'});
...
const beforeSearch:CollectionBeforeOperationHook=({args,operation,req})=>{
 ...
 if(!plan.rank && plan.statusSort===undefined) return;
 ranking.set(findArgs,{plan,original:{...findArgs}});
 // Rank lightweight candidates before pagination; hydrate only the requested page.
 findArgs.pagination=false; findArgs.limit=0; findArgs.page=1; findArgs.includeLockStatus=false; findArgs.select={[plan.title]:true,...(plan.statusSort!==undefined?{cmsStatus:true}:{})};
 if(plan.statusSort!==undefined) findArgs.sort="id";
};
const afterSearch:CollectionAfterOperationHook=async ({args,operation,result,req})=>{
 ...
 const docs=plan.statusSort!==undefined ? sortByCMSStatus(found.docs,plan.statusSort) : rankMatches(found.docs,plan.title,plan.terms);
 ...
 const hydrated=ids.length ? await req.payload.find({collection:plan.collection,req,overrideAccess:original.overrideAccess,draft:original.draft,depth:original.depth,locale:req.locale,fallbackLocale:req.fallbackLocale,
  ...
  pagination:false,where:{and:[original.where||{},{id:{in:ids}}]},sort:plan.statusSort!==undefined?"id":original.sort,
 }) : {docs:[]};
...
/** Sort the displayed status, with deterministic title/id ties across pages. */
export function sortByCMSStatus<T extends Record<string, unknown>>(docs:T[], descending=false):T[] {
 const label=(doc:T) => statusLabels[(doc.cmsStatus as {status:keyof typeof statusLabels})?.status] || '';
 return [...docs].sort((a,b) => (descending?-1:1)*label(a).localeCompare(label(b), 'en-AU')
  || String(a.title||'').localeCompare(String(b.title||''),'en-AU')
  || String(a.id).localeCompare(String(b.id),'en-AU',{numeric:true}));
}
